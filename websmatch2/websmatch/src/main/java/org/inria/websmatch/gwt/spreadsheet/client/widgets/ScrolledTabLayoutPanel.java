...
public class ScrolledTabLayoutPanel extends TabLayoutPanel {
    ...
    // added by Emmanuel Castanier INRIA
    @Override
    public void add(Widget child, String title, boolean html) {
	super.add(child, title, html);
	checkIfScrollButtonsNecessary();
    }
	...
	return new ClickHandler() {
		...
		if (newLeft <= 0
			&& (getTabBarWidth() - newLeft < (rightOfLastTab + 20))) {
		    scrollTo(newLeft);
		}
	    }
	};
	...
	scrollLeftButton.addClickHandler(createScrollClickHandler(+40));
	
	scrollLeftButton.setStyleName("scrollButton");
	...
	scrollRightButton.addClickHandler(createScrollClickHandler(-40));
	
	scrollRightButton.setStyleName("scrollButton");
