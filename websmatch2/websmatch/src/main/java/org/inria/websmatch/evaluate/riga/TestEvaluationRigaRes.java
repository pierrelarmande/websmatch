...
public class TestEvaluationRigaRes {
	...
	int[] indexes = new int[]{5509,5680,6037,6333,6418};
	...
	for (int i = 0; i < indexes.length-1; i++) {
		...
		String query = "SELECT ele1.name, ele2.name FROM match_results, elements as ele1, elements as ele2 WHERE expert = '1' AND id_element1 = ele1.id AND id_element2 = ele2.id AND id_element1 IN (SELECT id FROM elements WHERE schema_id = '"
	    ...
	    resToPrint.put(leftId+ " - "+rightId, resByTech);
