...
public class MatchGenerator {
	...
	for (Matcher matcher : matchers) {
	    ...
	    //
	    // ok, if the matcher is a YAM Matcher, don't merge scores
	    if (matcher instanceof YAMMatcherWrapper && matchers.size() == 1) {
		System.err.println("YAMMatcher detected, don't merge!");
		...
		for (ElementPair pair : scores.getElementPairs()) {
		    ...
		    MatcherScore score = scores.getScore(pair);
		    if (score.getTotalEvidence() <= 0)
			continue;
		    double positiveEvidence = score.getPositiveEvidence();

		    // Stores the match score if positive
		    if (positiveEvidence > 0)
			matchScores.setScore(pair.getSourceElement(), pair.getTargetElement(), positiveEvidence);
		}
		return matchScores;
	    }
	    // End of modif for YAM
	    //
